...
public class Test {
    ...
    public List<PoolMember> testUser() {
        ...
        Pool pool = new Pool("poolname1", "neigh1", "desc1", "95123");
        pool.setPoolLeader(u1);
        poolRepo.save(pool);
        //how does poolleader get tracked in poolmembers table?
        PoolMember p1 = new PoolMember(u1, u1.getId(), true,true, pool);
        PoolMember p2 = new PoolMember(u2, u1.getId(), true,true, pool);
        PoolMember p3 = new PoolMember(u3, u1.getId(), true,true, pool);
        PoolMember p4 = new PoolMember(u4, u1.getId(), true,true, pool);
        PoolMember p5 = new PoolMember(u5, u1.getId(), true,true, pool);
        poolMemberRepo.save(p1);
        poolMemberRepo.save(p2);
        poolMemberRepo.save(p3);
        poolMemberRepo.save(p4);
        poolMemberRepo.save(p5);
        return poolMemberRepo.findAll();
    }
